...
def validate_inferred_example_tuned(example, only_compiler_validation: bool = False):
    """
    Validates the finetuned code against the following validation classes:
    ...
    inference_code = strip_c_comments(example.get("finetuned_code", ""))
    actual_code = strip_c_comments(example.get("output", ""))
    task = example.get("input", "")

    finetuned_build_command = example.get("finetuned_build-command", "")
    #logger.error(f"finetuned build_command initial: {finetuned_build_command}")
    source_file_name = example.get("file-name", "")
    source_build_command = example.get("build-command", "")
    build_command, filename = select_build_command(finetuned_build_command, source_file_name, source_build_command)
    #logger.error(f"finetuned filename: {filename}")
    #logger.error(f"finetuned build_command: {build_command}")
    # Main Function Presence Check
    
    if not only_compiler_validation:
        if not has_main_function(inference_code):
            example["finetuned_has_main_function"] = False
        else:
            example["finetuned_has_main_function"] = True

        # Valid Include Directives Check
        if not has_valid_include(inference_code):
            example["finetuned_has_include_directives"] = False
        else:
            example["finetuned_has_include_directives"] = True

        # Must Have Libraries Check
        if not has_required_libraries(inference_code, C_RELEVANT_HEADERS):
            example["finetuned_has_required_libraries"] = False
        else:
            example["finetuned_has_required_libraries"] = True

        # CPP indicators Check
        valid, reasons = is_valid_c_code_with_no_cpp_indicator(inference_code)
        if valid:
            example["finetuned_has_no_cpp_indicators"] = True
            example["finetuned_cpp_indicators_details"] = None
        else:
            example["finetuned_has_no_cpp_indicators"] = False
            example["finetuned_cpp_indicators_details"] = reasons
        ...
        if violations:
            ...
            example["finetuned_has_no_restricted_headers_and_patterns"] = False
            example["finetuned_restricted_headers_and_patterns_details"] = full_message
        else:
            example["finetuned_has_no_restricted_headers_and_patterns"] = True
            example["finetuned_restricted_headers_and_patterns_details"] = None
        ...
        #example["finetuned_PercentageTaskRelevance_GEMINI"] = prepare_and_rate_sample_gemini(task, inference_code)
        example["finetuned_PercentageTaskRelevance_GEMINI"] = 0

        # Wait before retrying
        #time.sleep(BASE_DELAY + random.uniform(0, JITTER))

        # Check the code for relevancy against the already generated code - Gemini
        #example["finetuned_PercentageCodeRelevanceToReference_GEMINI"] = compare_and_rate_inferred_code_gemini(task, inference_code, actual_code)
        example["finetuned_PercentageCodeRelevanceToReference_GEMINI"] = 0
        ...
        #example["finetuned_PercentageTaskRelevance_GPT"] = prepare_and_rate_sample_gpt(task, inference_code)
        example["finetuned_PercentageTaskRelevance_GPT"] = 0
        # Wait before retrying
        #time.sleep(BASE_DELAY + random.uniform(0, JITTER))

        # Check the code for relevancy against the already generated code - GPT
        #example["finetuned_PercentageCodeRelevanceToReference_GPT"] = compare_and_rate_inferred_code_gpt(task, inference_code, actual_code)
        example["finetuned_PercentageCodeRelevanceToReference_GPT"] = 0
    ...
    example["finetuned_inference_has_valid_syntax"], example["finetuned_inference_syntax_check_message"], example["finetuned_inference_warnings"] = validate_syntax(inference_code, filename)

    # Advanced level checks:
    # Check the build of the inferred code for the task
    example["finetuned_inference_has_valid_build"], example["finetuned_inference_build_message"] = validate_build(inference_code, filename, build_command)
    example["finetuned_inference_static_code_quality_score"] = evaluate_code_quality_from_string(inference_code)
