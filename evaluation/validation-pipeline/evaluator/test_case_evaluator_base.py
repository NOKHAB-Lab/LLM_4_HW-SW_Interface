...
def evaluate_base_test_cases(validation_metrics: dict) -> dict:
    """
    Evaluate individual test cases and return test case results.
    
    Returns:
        dict with two keys:
        - 'base_test_case_validation': dict of 5 test case statuses
        - 'base_all_test_cases_passed': bool
    """
    logger.info(f"Starting test case validation for base code..........")
    # Extract booleans and int safely
    has_main = bool(validation_metrics.get("base_has_main_function", 0))
    no_restricted_headers = bool(validation_metrics.get("base_has_no_restricted_headers_and_patterns", 0))
    no_cpp_elements = bool(validation_metrics.get("base_has_no_cpp_indicators", 0))
    valid_syntax = bool(validation_metrics.get("base_inference_has_valid_syntax", 0))
    successful_build = bool(validation_metrics.get("base_inference_has_valid_build", 0))
    quality_rating = validation_metrics.get("base_inference_static_code_quality_score", 1)
