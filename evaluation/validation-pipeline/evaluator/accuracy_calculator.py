...
def calculate_code_accuracy(validation_metrics: dict) -> tuple[float, dict]:
    """
    ...
    has_main = validation_metrics.get("has_main_function", 0)
    uses_required_libraries = validation_metrics.get("has_required_libraries", 0)
    no_restricted_headers = validation_metrics.get("has_no_restricted_headers_and_patterns", 0)
    no_cpp_elements = validation_metrics.get("has_no_cpp_indicators", 0)
    has_include_directives = validation_metrics.get("has_include_directives", 0)
    valid_syntax = validation_metrics.get("inference_has_valid_syntax", 0)
    successful_build = validation_metrics.get("inference_has_valid_build", 0)
    relevancy_task_gemini = validation_metrics.get("PercentageTaskRelevance_GEMINI", 0)
    relevancy_task_gpt = validation_metrics.get("PercentageTaskRelevance_GPT", 0)
    relevancy_reference_gemini = validation_metrics.get("PercentageCodeRelevanceToReference_GEMINI", 0)
    relevancy_reference_gpt = validation_metrics.get("PercentageCodeRelevanceToReference_GPT", 0)
    quality_rating = validation_metrics.get("inference_static_code_quality_score", 1)  # Default to lowest rating
    compiler_warnings = validation_metrics.get("inference_warnings", 0)
