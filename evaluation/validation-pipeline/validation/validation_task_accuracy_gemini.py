...
def prepare_and_rate_sample_gemini(task_description, generated_code):
...
where 100% means a perfect match. Only respond with a single number followed by a percent sign, e.g., "92%"."""
    ...
    #write_to_txt('gemini_correct.txt', response)
    if response:
        logger.debug(f"Processing Gemini response of Code Correctness: '{response.strip()}'")
        ...
        if match and match.group(2):
            ...
            logger.info("No valid percentage found in Gemini response of Code Task Correctness.")
            return None
    else:
...
def compare_and_rate_inferred_code_gemini(task_description, inferred_code, valid_code):
    prompt = f"""Given the following task:
{task_description}
...
Only respond with a single number followed by a percent sign, e.g., "85%"."""
    ...
    response = call_gemini_api(prompt)
    #write_to_txt('gemini_compare.txt', response)
    if response:
        logger.debug(f"Processing GPT response of Gemini Comparison: '{response.strip()}'")
        match = re.search(r'(\b|[^0-9])?([1-9]?[0-9]|100)%(\b|[^0-9])?', response.strip())
        if match and match.group(2):
            percentage = int(match.group(2))
            if 0 <= percentage <= 100:
            ...
            logger.info("No valid percentage found in Gemini response of Code Comparison.")
            return None
    else:
        logger.error("No response from API.")
        return None
